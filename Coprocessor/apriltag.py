...
library used by ctypes.
    ...
    def detect(self, img, return_image=False):
        ...
        for i in range(0, detections.contents.size):
            ...
            tag_dict = {
                'tag_family': ctypes.string_at(tag.family.contents.name).decode('utf-8'),
                'tag_id': tag.id,
                'hamming': tag.hamming,
                'goodness': tag.goodness,
    ...
    def detection_pose(self, detection, camera_params, tag_size=1, z_sign=1):
        ...
        corners = np.array(detection["corners"]).flatten().astype(np.float64)
        ...
        pose_r = M[:3, :3]  # Extract rotation matrix
        pose_t = M[:3, 3]  # Extract translation vector

        return pose_r, pose_t, init_error.value, final_error.value
